...
public class Cell {
    ...
    PApplet parent;
    public int bodyShape; // 0: potatoid, 1: triangleRing, 2: clam.
    public float mutate;
    public int numLegs;
    public float x, y;
    public float vx, vy;
    ...
    public boolean isOnScreen() {
        ...
        if ((x > parent.width) || (x < 0) || (y > parent.height) || (y < 0)) {
            if (this.stayOnScreen) {
                // push cell away from screen border              
                this.ax = ( this.x - (parent.width / 2)) / parent.width;
                this.ay = ( this.y - (parent.height / 2)) / parent.height;                
                return true;
            } else {
                return false;
            }
